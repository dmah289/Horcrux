...
Kế hoạch bổ sung 9 họ curve còn thiếu cho `AnimationHelper`, bám đúng pattern của
`Curves/QuadEase.cs`. Sau khi thêm file curve, cập nhật `switch` trong
`EaseInterpolator.GetEaseValue`.
...
- `t` đã được clamp `[0,1]` bởi `EaseInterpolator` → **không clamp lại** trong curve.
...
## Cập nhật `EaseInterpolator.GetEaseValue`
...
- [ ] Bổ sung 27 case vào `switch` của `EaseInterpolator`.
