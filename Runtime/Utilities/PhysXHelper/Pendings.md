...
Đã có: `HarmonicOscillator` (dao động điều hòa đơn giản, Sin/Cos).
...
### `Easing`  ⭐ ưu tiên cao
- Bộ hàm easing (EaseInOutCubic, EaseOutBack, Elastic, Bounce…).
- Đầu vào `t ∈ [0,1]`, đầu ra đã cong. Xương sống của mọi animation không dùng tween library.
...
1. **`Easing`** — bộ hàm cong `t∈[0,1]`. Nền của mọi animation.
2. **`InterpolationHelper`** — `Remap`, `SmoothStep`, exp-decay lerp độc lập framerate.
3. **`AngleHelper`** — chuẩn hóa góc, shortest-delta, xoay vector 2D.
4. **`GeometryHelper`** — khoảng cách, closest-point, giao điểm (dùng cho stagger/snap).
5. **`RandomHelper`** — gaussian, weighted, jitter, shuffle (dùng cho shake/granular).
6. `HarmonicOscillator` — ✅ đã có.
...
`Easing` → `SpringDamper` → `SquashStretch` → `Shake` → `AudioPitchHelper` + `HapticHelper` → `StaggerHelper` → `MagneticSnap` → `GranularSettle` → `SatisfyingClear`.
