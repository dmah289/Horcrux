...
namespace Horcrux.Runtime.Utilities.PhysXHelper
{
    ...
    public static class DampedOscillator
    {
        ...
        /// <summary>
        /// Calculate oscillator's current envelop amplitude
        /// </summary>
        /// <remarks>Formula : E = A * e^(-λt)</remarks>>
        /// <param name="decay">Decay factor (1/s). If non-positive value -> return amplitude</param>
        /// <param name="amplitude">Initial amplitude</param>
        /// <returns>Current envelop amplitude</returns>
        ...
        /// <param name="decay">Decay factor λ (1/s). Non-positive value -> Harmonic Oscillator</param>
        /// <param name="amplitude">Initial amplitude</param>
        /// <param name="phaseShift">Initial phase</param>
        /// <returns>Instantaneous displacement at time t, is in the range of [-Envelope,Envelope]</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>
        /// Instantaneous velocity (Derivative of displacement) of the damping oscillator.
        /// </summary>
        /// <remarks>
        ...
        public static float GetDisplacementHalfLife(WaveStyle waveStyle, float frequency, float invHalfLife, float time, 
            float amplitude = 1f, float phaseShift = 0f)
            => invHalfLife <= 0f ? 0f : GetDisplacement(waveStyle, frequency, Ln2 * invHalfLife, time, amplitude, phaseShift);
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float GetVelocityHalfLife(WaveStyle waveStyle, float frequency, float invHalfLife, float time,
            float amplitude = 1f, float phaseShift = 0f)
            => invHalfLife <= 0f ? 0f : GetVelocity(waveStyle, frequency, Ln2 * invHalfLife, time, amplitude, phaseShift);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float GetEnvelopeHalfLife(float invHalfLife, float time, float amplitude = 1f)
            => invHalfLife <= 0f ? 0f : GetEnvelope(Ln2 * invHalfLife, time, amplitude);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float GetSettlingTimeHalfLife(float invHalfLife, float threshold = 0.05f)
            => invHalfLife <= 0f ? float.PositiveInfinity : GetSettlingTime(Ln2 * invHalfLife, threshold);
    }
}