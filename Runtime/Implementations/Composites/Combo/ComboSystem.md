...
> **Loại tài liệu:** Plan — developer tự code lại để nắm logic. `.md` thiết kế + `.html` viết **sau** khi có source; lúc đó tài liệu của hệ mới coi là đủ.
>
...
| Editor-first | Luật đứt + hệ số nhân + mốc bậc đều là **tham số trong Inspector**; màu/thanh/pop gán ở Inspector. Không hằng số feel nào hardcode (§0.5) |
...
| Giữ `IComboWindowPolicy` dù có thể gộp 3 công thức | Có **3 implementation** ⇒ interface đáng tồn tại. Và đây là abstraction duy nhất user chỉ định rõ ("3 cơ chế cắm được") — cắt nó là cắt vào mục đích ban đầu |
...
| Nhận `int[] tierMinCounts` thay `IComboTierTable` | Interface + SO + struct `ComboTier` đều chỉ để chở **một** `int` mỗi bậc ⇒ không gọi được tên chỗ hỏng nếu xóa |
...
| **Enum + tham số** trong Inspector | Mọi thứ quyết được lúc authoring thì để ở Inspector. `InterfaceReference<T>` chưa tồn tại nên serialize policy trực tiếp là không khả thi |
...
| Quyết định | Lý do |
|---|---|
| Tồn tại **vì** chưa có game caller | Mọi hàm phải có caller thật. Không có Task này thì Task 1–5 là code chưa ai gọi |
